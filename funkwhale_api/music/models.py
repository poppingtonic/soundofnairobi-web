...
class Track(APIModelMixin):
    title = models.CharField(max_length=255)
    artist = models.ForeignKey(Artist, related_name="tracks", on_delete=models.CASCADE, null=True, blank=True)
...
class InventoryNumberSeq(models.Model):
    """This class maps to inventory_number_seq which  is a PostgreSQL sequence,
       This sequence runs from 0 to 9999 after which it restarts
    """
    ...
    class Meta:
        db_table = u'inventory_number_seq'

def get_next_value():
  inventory_number = InventoryNumberSeq.objects.raw("select nextval('inventory_number_seq')")[0]
  return "{}".format(inventory_number.nextval)
...
class Upload(models.Model):
    ...
    inventory_number = models.CharField(max_length=20, default=get_next_value, editable=False, null=True, blank=True)
    duration = models.IntegerField(null=True, blank=True)
    recording_start_location = models.TextField(null=True, blank=True)
    location_latitude = models.FloatField(null=True, blank=True)
    location_longitude = models.FloatField(null=True, blank=True)
    recording_device = models.TextField(null=True, blank=True)
    recording_route = models.TextField(null=True, blank=True)
    recording_description = models.TextField(null=True, blank=True)
    recording_name = models.TextField(null=True, blank=True)
    recording_internal_name = models.TextField(null=True, blank=True)
    recording_thoughts = models.TextField(null=True, blank=True)
    recording_special_sounds = models.TextField(null=True, blank=True)
    ...
    @property
    def filename(self):
        return "SON{}.{}".format(self.inventory_number.zfill(6), self.extension)
    ...
    def save(self, **kwargs):
        ...
        if not self.import_details:
            self.import_details = {"code": "success"}
        if not self.metadata:
            self.metadata = {}
        if not self.import_metadata:
            self.import_metadata = {"funkwhale": {"config": {"replace": False, "dispatch_outbox": False, "broadcast": False}}}
        return super().save(**kwargs)
